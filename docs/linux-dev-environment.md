...
- **Laptop SSH:** `nico@192.168.4.44` (DHCP — IP may change)
- **Mac Studio:** `192.168.4.179` (en1, WiFi)
- **Auth:** ED25519 key from Mac Studio (passwordless)

> **Tip:** Consider assigning a static IP or DHCP reservation on your router to avoid IP drift.
...
- [x] Git remote `linux` added on Mac Studio → `nico@192.168.4.44:~/butane_flutter.git`
...
- [ ] DHCP reservation for laptop (avoid IP drift)
...
git remote add linux nico@<laptop-ip>:~/butane_flutter
...
git push linux main
ssh nico@<laptop-ip> "cd ~/butane_flutter && export PATH=\$HOME/flutter/bin:\$PATH && flutter build linux"
```
