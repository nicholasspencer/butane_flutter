...
void main() {
  test('launches profile macOS peripheral and scrapes its sentinel', () async {
    final fixture = await _Fixture.create(
      sentinel: 'GRID_VM_URI=ws://127.0.0.1:6123/follower/ws',
    );
    ...
    expect(fixture.starter.arguments, [
      'run',
      '--profile',
      '-d',
      'macos',
    ...
    expect(daemon.endpoint.vmServiceUri, 'ws://127.0.0.1:6123/follower/ws');
    expect(daemon.endpoint.station, 'macos-follower');
  ...
  test('readiness timeout kills the exact spawned process', () async {
    ...
    addTearDown(fixture.dispose);
    final launcher = MacosFollowerLauncher(
    ...
    for (var attempt = 0; attempt < 100 && alive; attempt++) {
      ...
      if (alive) await Future<void>.delayed(const Duration(milliseconds: 10));
    }
...
final class _Fixture {
  ...
  final MacosFollowerLauncher launcher;
...
''');
  ...
  Future<void> dispose() async {
    ...
    if (process != null) Process.killPid(process.pid, ProcessSignal.sigkill);
