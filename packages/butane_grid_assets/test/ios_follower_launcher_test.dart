...
typedef _SignaledProcessGroup = ({int pgid, ProcessSignal signal});

class _IosProcessFixture {
  _IosProcessFixture(this.source, {this.startError});
...
class _FakeProcessGroupController implements ProcessGroupController {
  final resolvedPids = <int>[];
  final groupSignals = <_SignaledProcessGroup>[];
  ...
  bool signalGroup(int pgid, ProcessSignal signal) {
    groupSignals.add((pgid: pgid, signal: signal));
    return true;
  }
}
...
''';
  ...
  group('launch strategy and console parsing are explicit and loud', () {
    test(
...
''');
    ...
    test(
      'accepts current and legacy iOS readiness extension prefixes',
      () async {
        var extensionRpc = 'ext.exploration.butane.wait_for_state';
        ...
        extensionRpc = 'ext.leonard.butane';
        expect(await isIosExplorationReady(base), isTrue);
...
''');
      ...
      expect(start.executable, '/usr/bin/script');
      expect(start.arguments, <String>[
        '-t',
        '0',
        '/dev/null',
      ...
      expect(start.mode, ProcessStartMode.detachedWithStdio);
      final bundleIndex = start.arguments.indexOf(
        'com.nicospencer.butaneHarness',
      );
      expect(start.arguments.sublist(bundleIndex + 1), <String>[
        '--vm-service-host=0.0.0.0',
        '--enable-dart-profiling',
      ]);
...
''';
    ...
    test(
      'console parser ignores malformed banners until a valid banner',
      () async {
        final harness = await Directory.systemTemp.createTemp(
          'ios_follower_devicectl_malformed_console_',
        );
        addTearDown(() => harness.delete(recursive: true));
        await Directory(
          '${harness.path}/build/ios/iphoneos/Runner.app',
...
''');
    ...
    group('devicectl failures are loud and reaped', () {
      ...
      test('missing script wrapper names the devicectl console', () async {
        final harness = await Directory.systemTemp.createTemp(
          'ios_follower_script_failure_',
        );
        addTearDown(() => harness.delete(recursive: true));
        await Directory(
          '${harness.path}/build/ios/iphoneos/Runner.app',
        ...
        final fixture = _IosProcessFixture(
          _hangingHelper,
          startError: ProcessException('/usr/bin/script', const <String>[]),
        );
        addTearDown(fixture.dispose);
        final launcher = IosFollowerLauncher(
          deviceId: 'device',
          ...
          throwsA(
            isA<StateError>()
                .having(
                  (error) => '$error',
                  'wrapper path',
                  contains('/usr/bin/script'),
                )
                .having(
                  (error) => '$error',
                  'console purpose',
                  contains('devicectl console'),
                ),
          ),
        );
        expect(fixture.processCount, 0);
        expect(fixture.starts.single.executable, '/usr/bin/script');
      });
...
''');
            ...
            throwsA(
              isA<StateError>()
                  .having(
                    ...
                    'exit',
                    contains(
                      'devicectl via /usr/bin/script exited before readiness',
                    ),
                  )
                  .having(
                    (error) => '$error',
          ...
          expect(await fixture.allChildrenExited, isTrue);
          expect(processes.resolvedPids, hasLength(1));
          expect(processes.resolvedPids.single, greaterThan(0));
          expect(processes.groupSignals, hasLength(1));
          expect(
            processes.groupSignals.single.pgid,
            processes.resolvedPids.single,
          );
          expect(processes.groupSignals.single.signal, ProcessSignal.sigkill);
        },
      );
    });
  });
